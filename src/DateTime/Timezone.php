...
enum Timezone: string
{
    ...
    public static function fromString(string|\Stringable $value): Timezone
    {
        // This way we support special case GMT and UTC but keep format standard.
        $normalized = StringValue::fromString($value);

        // Support 'Z' format.
        if ($normalized->equals(StringValue::fromString('Z'))) {
            $normalized = StringValue::fromString(self::ETC_UTC->value);
        }

        // Support 'UTC' and 'GTM' format.
        if (!$normalized->contains('/')) {
            $normalized = $normalized->prepend('Etc/');
        }

        return self::from($normalized->toString());
    }
}
